...
def place_antinode(antennas, vectors):
    antinode_map[antennas[0][0]][antennas[0][1]] = "#"
    antinode_map[antennas[1][0]][antennas[1][1]] = "#"

    antinode1_pos = antennas[0] + vectors[0]
    antinode2_pos = antennas[1] + vectors[1]

    while True:
        # print(f"{antinode1_pos}, {antinode2_pos}")

        is_antinode1_inside = is_inside_map(antinode1_pos)
        is_antinode2_inside = is_inside_map(antinode2_pos)

        if not is_antinode1_inside and not is_antinode2_inside:
            break

        if is_antinode1_inside:
            antinode_map[antinode1_pos[0]][antinode1_pos[1]] = "#"

        if is_antinode2_inside:
            antinode_map[antinode2_pos[0]][antinode2_pos[1]] = "#"

        antinode1_pos = antinode1_pos + vectors[0]
        antinode2_pos = antinode2_pos + vectors[1]
